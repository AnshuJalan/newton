...
contract PutOption is UsingDAI, UsingPriceFeed{
    ...
    */
    ...
    mapping(uint => Option) public idToOption;
    mapping(address => uint[]) public addressToOptions;
    
    constructor(address _daiAddress, address payable _tellorAddress) UsingDAI(_daiAddress) UsingPriceFeed(_tellorAddress) public{
    ...
    */
    function createNewOptionLong(
        ...
        uint _strikePrice, 
        uint _cost, 
        uint _quantity) external{
            
            bool _costEscrowed;
            _costEscrowed = daiInstance.transferFrom(msg.sender, address(this), _cost);
            require(_costEscrowed, "Insuffcient amount approved for the option to transfer");
    ...
    */
    function createNewOptionShort(
        ...
        uint _initialMargin
            ...
            bool _marginEscrowed = daiInstance.transferFrom(msg.sender, address(this), _initialMargin);
            require(_marginEscrowed, "Insuffcient amount approved for the option to transfer");
    ...
    * @dev Must be preceded by a approve call to DAI token contract, during implementation
    */
    function buyToOpen(uint _id, uint _type) external isAvailable(idToOption[_id]){
        ...
        bool _premiumSent;
        _premiumSent = daiInstance.transferFrom(msg.sender, address(this), option.cost);
    ...
    */
    function sellToOpen(uint _id, uint _initialMargin) external isAvailable(idToOption[_id]){
        
        //fetch option from mapping
        Option storage option = idToOption[_id];
    
        //Check if a valid option is being selected
        require(option.openPosition == POSITION.SHORT, "Wrong option selected!");
        require(msg.sender != option.partyL, "You selected your own option!");
       
        require(_initialMargin >= option.strikePrice, "Insufficient margin provided");
    ...
    */
    function settleOption(uint _id) external payable hasNotSettled(idToOption[_id]) {
        
        Option storage option = idToOption[_id];

        
        uint marketVal = option.quantity.mul(getMarketValue());
        ...
        if(option.settlementType == 0){
            //ETH settlement
            require(msg.value == option.quantity, "Insuffcient DAI approved for transfer!");
            ...
            //DAI settlement
            uint profit = option.strikePrice.sub(marketVal);
            daiInstance.transfer(msg.sender, profit);
            
            //remaining DAI back to Seller
            uint remaining = option.initialSellerMargin.sub(profit);
    ...
    */
    function closeOption(uint _id) external notFilledorExpired(idToOption[_id]){
    ...
    */
    function refundExpired(uint _id) external isFilledandExpired(idToOption[_id]){
    ...
    */
    function getOptionsCount(address _address) public view returns(uint){
        return addressToOptions[_address].length;
    }
    
    modifier notFilledorExpired(Option memory option){
        require(option.isAvailable, "Option positions already filled or option no longer valid!");
