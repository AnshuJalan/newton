...
class NewOrder extends Component {
      ...
      try {
        ...
        if (this.state.position == 'long') {
          ...
          await this.state.callContract.methods
            .createNewOptionLong(sp, cost, qty)
            .send({
              from: this.state.accounts[0],
            });
          ...
          if (type == 1) {
            ...
            //Approval
            await this.state.daiContract.methods
              .approve(this.state.callContract.options.address, marginT)
              .send({
                from: this.state.accounts[0],
              });
            ...
            //Create the Call Option
            await this.state.callContract.methods
              .createNewOptionShort(type, sp, cost, qty, marginT)
              .send({
                from: this.state.accounts[0],
              });
        ...
        this.setState(
          { loading: false, strikeprice: '', cost: '', quantity: '' },
          this.setLong
        );
      } catch (err) {
      ...
      try {
        ...
        if (this.state.position == 'long') {
          ...
          await this.state.putContract.methods
            .createNewOptionLong(type, sp, cost, qty)
            .send({
              from: this.state.accounts[0],
            });
          ...
          //Create the Call Option
          await this.state.putContract.methods
            .createNewOptionShort(sp, cost, qty, sp)
            .send({
              from: this.state.accounts[0],
            });
  ...
  validate = async (e) => {
    ...
    if (
      this.state.strikeprice <= 0 ||
      this.state.quantity <= 0 ||
      this.state.cost <= 0
    ) {
      this.setState({ warnModalShow: true });
    } else {
      let marketVal = await this.state.priceContract.methods
        .getMarketValue()
        .call();
  ...
  render() {
    ...
    return (
      <div className='new-order sec-bg text-white text-center'>
        <div className='input-box-header'>New Order</div>
          ...
          <div style={{ margin: 'auto' }} className='row text-center mt-3'>
            ...
            <div className='col-sm-4'>
              <select
                ...
                className='float-right option-select'
              >
          ...
          <div className='text-left mt-4'>
            <form onSubmit={this.onSubmit} ref={(f) => (this.form = f)}>
              ...
              <Button
                ...
                className={
                  'mt-1 text-white font-weight-bold btn btn-block ' +
        ...
        >
          ...
          <Modal.Body className='sec-bg'>
            Please enter a valid amount for all the fields!
          </Modal.Body>
        ...
        >
            ...
            {this.state.position == 'long' && (
              <span>
                ...
                <img
                  ...
                  className='mx-1'
                />
                {this.state.cost} DAI from your account, to be paid as premium
                to the seller.
              </span>
            )}
              ...
              this.state.type == '1' && (
                <span>
                  ...
                  <img
                    ...
                    className='mx-1'
                  />
                  {(0.67 * this.state.marketVal).toFixed(2)} DAI from your
                  account, as margin.
                </span>
              )}
            {this.state.selectedOption == 'call' &&
              ...
              this.state.type == '0' && (
                <span>
                  ...
                  <img
                    ...
                    className='mx-1'
                  />
                  {this.state.quantity} ETH from your account, to be given to
                  the buyer at settlement.
                </span>
              )}
            {this.state.selectedOption == 'put' &&
              this.state.position == 'short' && (
                <span>
                  Please approve the options contract to retrieve{' '}
                  <img
                    ...
                    className='mx-1'
                  />
                  {(1 * this.state.strikeprice).toFixed(2)} DAI from your
                  account, as margin.
                </span>
              )}
          </Modal.Body>
          <Modal.Footer className='head-bg'>
            <Button
              variant='danger'
              onClick={() => this.setState({ mainModalShow: false })}
            >
              Cancel
            </Button>
            <Button
